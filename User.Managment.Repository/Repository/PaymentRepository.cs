...
namespace User.Managment.Repository.Repository
{
    public class PaymentRepository : IPaymentRepository
    {
        ...
        public PaymentRepository(ApplicationDbContext db, RoleManager<IdentityRole> roleManager, UserManager<ApplicationUser> userManager, IEmailRepository emailRepository, PaypalSettings paypalConfig, FrontSettings frontUrl, WhatsappSettings whatsappSettings)
        {
            _db = db;
        ...
        public async Task<ResponseDto> ConfirmPaypalAsync(string token)
        {
            try
            {
        ...
        public async Task<ResponseDto> CreateOrderAsync(ConfirmOrder confirmOrder)
        {
            try
            {
        ...
        public async Task<ResponseDto> PaypalCardAsync(Order model)
        {
            try
            {
        ...
        private async Task WhatsappNotificacion(Venta ventaDto)
        {
            ...
            HttpRequestMessage request = new(HttpMethod.Post, "https://graph.facebook.com/v21.0/" + idTelefono + "/messages");
            request.Headers.Add("Authorization", "Bearer " + token);
